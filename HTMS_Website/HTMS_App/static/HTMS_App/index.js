...
/* harmony export */   "disableNonEssElements": () => (/* binding */ disableNonEssElements),
/* harmony export */   "profileRandomColor": () => (/* binding */ profileRandomColor),
/* harmony export */   "scroller": () => (/* binding */ scroller),
/* harmony export */   "searchUsersAndTickets": () => (/* binding */ searchUsersAndTickets),
...
function _post_to_databae() {
  _post_to_databae = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee4(el_id, el, status_element, csrftoken, slugify) {
...
var searchUsersAndTickets = function searchUsersAndTickets() {
  var searchBoxes = [{
    'name': 'user',
  ...
  searchBoxes.forEach(function (el) {
    var boxEl = document.querySelector(el.search_box);
    try {
      boxEl.addEventListener('click', function (e) {
        return searchUsersClick(e, boxEl, el);
      });
    } catch (error) {
...
var searchUsersClick = function searchUsersClick(e, boxEl, el) {
  e.preventDefault();
  var searchVal = boxEl.querySelector('.search-box-input').value;
...
var status_update_asset = /*#__PURE__*/function () {
  var _ref3 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee3(search_value, el) {
    return _regeneratorRuntime().wrap(function _callee3$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          case 0:
            _context3.next = 2;
            return fetch("".concat(el.url).concat(search_value)).then(function (response) {
              return response.json();
            }).then(function (data) {
              return updateSelectTag(data, el);
            });
          case 2:
          case "end":
  ...
  return function status_update_asset(_x11, _x12) {
    return _ref3.apply(this, arguments);
  };
}();
var updateSelectTag = function updateSelectTag(data, el) {
  data = Object.values(data);
  var selectTagEl = document.querySelector(el.selectHtmlTagID);
  var selectTagElParent = selectTagEl.previousElementSibling;
  selectTagEl.remove();
  var html = '';
  if (el.name == 'user') {
    html += "<select  name=\"search_user_selection\" class=\"select\" id=\"".concat(el.selectHtmlTagID.toString().slice(1), "\">\n              <option disabled selected hidden value=\"\"> --").concat(data.length > 0 ? 'Select User (' + data.length : ' No Data ( No', " Users Found) -- </option> \n              <option value=\"0\"> &#128472 Reset User's Fields</option>");
    data.forEach(function (user) {
      html += "<option value=\"".concat(user.id, "\">").concat(user.first_name, " ").concat(user.last_name, " (").concat(user.username, ")</option>");
    });
  }
  if (el.name == 'ticket') {
    html += "<select  name=\"search_ticket_selection\" class=\"select\" id=\"".concat(el.selectHtmlTagID.toString().slice(1), "\">\n          <option disabled selected hidden value=\"\"> --").concat(data.length > 0 ? 'Select Ticket (' + data.length : ' No Data ( No', " Tickets Found) -- </option> \n          <option value=\"0\"> &#128472 Reset Ticket's Fields</option>");
    data.forEach(function (ticket) {
      html += "<option value=\"".concat(ticket.id, "\">No. ").concat(ticket.id, " | ").concat(ticket.requester_name, " | ").concat(ticket.request_category, "</option>");
    });
  }
  html += "</select>";
...
var UserFormUpdate = function UserFormUpdate(data, el) {
  var selectTagEl = document.querySelector(el.selectHtmlTagID);
  var userForm = [];
  var ticketForm = [];
  selectTagEl.addEventListener('change', function () {
    if (el.name == 'user') {
      if (selectTagEl.value == 0) userForm.forEach(function (el) {
        return userFormReset(el);
      });
      if (selectTagEl.value > 0) {
        var dataUser = data.find(function (user) {
          return user.id == selectTagEl.value;
        });
        var userFormElId = get_elementsID(dataUser, el.name);
        userFormElId.forEach(function (el_id) {
          return userFormUpdation(el_id.id, el_id.value, userForm);
        });
      }
    }
    if (el.name == 'ticket') {
      if (selectTagEl.value == 0) ticketForm.forEach(function (el) {
        return userFormReset(el);
      });
      if (selectTagEl.value > 0) {
        var dataTicket = data.find(function (ticket) {
          return ticket.id == selectTagEl.value;
        });
        var ticketFormElId = get_elementsID(dataTicket, el.name);
        ticketFormElId.forEach(function (el_id) {
          return userFormUpdation(el_id.id, el_id.value, ticketForm);
        });
      }
    }
  });
};
var userFormUpdation = function userFormUpdation(selectorName, inputValue) {
  var userForm = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  if (selectorName == "#request_status") {
    var _element = document.querySelector(selectorName);
    _element.classList.add('disabled');
    _element.value = inputValue;
    if (userForm) userForm.push(_element);
    return;
  }
...
var get_elementsID = function get_elementsID(data, request_type) {
  if (!data) {
    data = {};
  }
  if (request_type == 'ticket') {
    return [{
      ...
      'id': '#request_mode',
      'value': data.request_mode
    }, {
      'id': '#request_priority',
      'value': data.request_priority
    }, {
      'id': '#request_category',
      'value': data.request_category
    }, {
      'id': '#request_technician',
      'value': data.request_technician_id
    }, {
      'id': '#subject',
      'value': data.subject
    }, {
  ...
  if (request_type == 'user') {
    return [{
      'id': '#name',
      'value': "".concat(data.first_name, " ").concat(data.last_name)
    }, {
      'id': '#requester_pr_number',
  ...
  try {
    ...
    selectCurrentStatus.addEventListener('change', function () {
      if (selectCurrentStatus.value != 'In Use') {
        ...
        elementIds.forEach(function (elIds) {
          return userFormUpdation(elIds.id, "");
        });
      ...
      if (selectCurrentStatus.value == 'In Use') {
        ...
        _elementIds.forEach(function (elIds) {
          var elementId = document.querySelector(elIds.id);
...
var submitButtonPosition = function submitButtonPosition(targetClass) {
...
_helper_js__WEBPACK_IMPORTED_MODULE_0__.disableNonEssElements(); // on Add Asset Page

_helper_js__WEBPACK_IMPORTED_MODULE_0__.searchUsersAndTickets();
